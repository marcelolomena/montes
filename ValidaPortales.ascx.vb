...
Partial Class ValidaPortales
    Inherits System.Web.UI.UserControl
    '------------------------------------------------------------
    ' Código generado por ZRISMART SF el 29-08-2011 8:52:10
    '------------------------------------------------------------
    ...
    '----------------------------------------
    Dim txtMasterName As TextBox ' Etiqueta : Código Comuna - Control : txtMasterName - Variable : ComunasCodigo
    '----------------------------------------
    ' Declaración de Campos de la Aplicación
    '----------------------------------------
    Dim PortalesName As String ' Etiqueta : Código Comuna - Control : txtMasterName - Variable : ComunasCodigo
    '----------------------------------------
    Protected Sub RFC_Login(ByVal sender As Object, ByVal e As System.EventArgs) Handles LoginButton.Click
        ...
        Try
            ...
            Else
                t = AccionesABM.MasterPartialInsert("PortalesId", "PortalesName", "Portales", MasterName, CLng(Session("PersonaId")), MasterId)
                Url = Url & "&Id=" & MasterId & "&MenuOptionsId=" & Request.QueryString("MenuOptionsId")
                Response.Redirect(Url, True)
            End If
        Catch
