...
Partial Class ValidaUsuarios
    Inherits System.Web.UI.UserControl
    '------------------------------------------------------------
    ' Código generado por ZRISMART SF el 15-03-2011 8:33:04
    '------------------------------------------------------------
    ...
    '----------------------------------------
    Dim txtMasterName As TextBox ' Etiqueta : Nombre de Usuario - Control : txtMasterName - Variable : UsuariosCodigo
    '----------------------------------------
    ' Declaración de Campos de la Aplicación
    '----------------------------------------
    Dim UsuariosCodigo As String ' Etiqueta : Nombre de Usuario - Control : txtMasterName - Variable : UsuariosCodigo
    '----------------------------------------
    Protected Sub RFC_Login(ByVal sender As Object, ByVal e As System.EventArgs) Handles LoginButton.Click
        ...
        Try
            ...
            If MasterId > 0 Then
                If Usuarios.LeerEstadoUsuarios(MasterId) = True Then
                    Url = Url & "&Id=" & MasterId & "&MenuOptionsId=" & Request.QueryString("MenuOptionsId")
                    Response.Redirect(Url)
                Else
                    MyMessage1.Text = "El usuario se encuentra inactivado, consulte con el administrador del sistema"
                End If
            Else
                t = AccionesABM.MasterPartialInsert("UsuariosId", "UsuariosCodigo", "Usuarios", MasterName, CLng(Session("PersonaId")), MasterId)
                Url = Url & "&Id=" & MasterId & "&MenuOptionsId=" & Request.QueryString("MenuOptionsId")
                Response.Redirect(Url)
            End If
        Catch
